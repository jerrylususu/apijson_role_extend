...
public class APIJSONVerifier extends AbstractVerifier<Long> {
	...
	public static JSONObject initAccess(boolean shutdownWhenServerError, APIJSONCreator creator, JSONObject table) throws ServerException {
		...
		for (int i = 0; i < size; i++) {
			...
			Map<RequestMethod, IRequestRole[]> map = new HashMap<>();
			map.put(RequestMethod.GET, convertStringToIRequestRole(JSON.parseObject(item.getString("get"), String[].class)));
			map.put(RequestMethod.HEAD, convertStringToIRequestRole(JSON.parseObject(item.getString("head"), String[].class)));
			map.put(RequestMethod.GETS, convertStringToIRequestRole(JSON.parseObject(item.getString("gets"), String[].class)));
			map.put(RequestMethod.HEADS, convertStringToIRequestRole(JSON.parseObject(item.getString("heads"), String[].class)));
			map.put(RequestMethod.POST, convertStringToIRequestRole(JSON.parseObject(item.getString("post"), String[].class)));
			map.put(RequestMethod.PUT, convertStringToIRequestRole(JSON.parseObject(item.getString("put"), String[].class)));
			map.put(RequestMethod.DELETE, convertStringToIRequestRole(JSON.parseObject(item.getString("delete"), String[].class)));
